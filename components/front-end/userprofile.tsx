    ...
    <>
      ...
      <div className='w-full flex flex-col justify-center items-center'>
        <div className='flex flex-col items-center gap-3'>
          ...
          />
          <h2 className='font-black text-3xl uppercase'>{user?.userHandle}</h2>
        ...
        {user?.links.map((item: any) => {
          const renderIcon = (title: string) => {
            ...
            } else if (title.toLowerCase().includes("dribble")) {
              return <Dribbble className='w-6 h-6' />;
            } else if (title.toLowerCase().includes("behance")) {
              return <Globe className='w-6 h-6' />;
            } else if (title.toLowerCase().includes("tiktok")) {
              return <Globe className='w-6 h-6' />;
            } else if (
              title
                .toLowerCase()
          ...
          return (
            ...
            >
              ...
              >
                <span>{renderIcon(item.socialMediaName as string)}</span>

                <span className='Capitalize'>Checkout {item.title}</span>
              </Link>
            </div>
          );
        })}
        ...
        <div className='space-y-2 mt-8 max-w-xl'>
          <div className='space-y-3'>
            <h2 className='font-bold mb-2 text-center'>
              Subscribe to {user?.userHandle} YT channel
            </h2>
          </div>
          {otherLinks.map((item) => {
            return (
